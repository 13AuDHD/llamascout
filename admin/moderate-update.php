...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $points = llama_points_policy_required(
            $db,
        ...
        if ($action === 'approve') {
            $contributionId = moderation_approve_update(
                $db,
                $updateId,
                (int) $adminUser['id'],
                $notes,
...
$original = is_array($item['original'] ?? null)
    ? $item['original']
    : [];

$photos = is_array($item['photo_list'] ?? null)
    ? $item['photo_list']
    : [];

$updatePointValue = llama_points_policy_required(
    $db,
    'approved_place_update'
);
...
$historyRow = llama_place_update_fetch_row(
    $db,
    $updateId
);

$history = $historyRow
    ? llama_place_update_history($historyRow)
    : [];
...
<section class="admin-update-changes">
    ...
    <?php foreach ($groupedChanges as $group): ?>
        <?php
        $meta = $group['meta'];
                ...
                <?php foreach ($changes as $path => $change): ?>
                    ...
                    <article class="admin-update-change-card">
                        ...
                        <div class="admin-update-before-after">
                            <div class="is-before">
                                <span>Current when submitted</span>
                                <strong><?= $e($oldDisplay) ?></strong>
                            </div>
    ...
    <section class="admin-update-history">
        <header>
            <div>
                ...
                <h2>What happened to this update</h2>
            </div>
        </header>
        ...
        <div class="admin-update-timeline">
            ...
            <?php foreach ($history as $event): ?>
                ...
                $title = match ($type) {
                    ...
                    'changes-requested' => 'Moderator requested changes',
                ...
                <article class="admin-update-timeline-event">
                    ...
                    <div>
                        ...
                        <?php if (
                            $type === 'resubmitted'
                            && !empty($event['proposal_changes'])
                        ): ?>
                            <div class="admin-update-history-diff">
                                <span>Changed on resubmission</span>
                                ...
                                <ul>
                                    <?php foreach (
                                        ...
                                        $beforeText =
                                            !empty($difference['before_present'])
                                                ? llama_place_update_display_value(
                                                    $path,
                                                    $difference['before'],
                                        ...
                                        $afterText =
                                            !empty($difference['after_present'])
                                                ? llama_place_update_display_value(
                                                    $path,
                                                    $difference['after'],
                        ...
                        <?php if (
                            isset($event['photo_count_before'])
                            || isset($event['photo_count_after'])
                        ): ?>
                        ...
                        <?php elseif (isset($event['photo_count'])): ?>
                            <p class="admin-update-history-meta">
                                Photos:
    ...
    <section class="admin-update-photos">
        ...
        <div class="admin-moderation-photo-grid">
            <?php foreach ($photos as $photo): ?>
                <?php
                $src = moderation_photo_path($photo);
