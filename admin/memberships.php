...
function membership_admin_local_to_utc(
    string $value,
...
function membership_admin_utc_to_input(
    ?string $value
): string {
    if (!$value) {
        return '';
...
function membership_admin_money(
    int $cents
): string {
    return
        '$'
...
function membership_admin_require_schema(
    PDO $db
): void {
    $requiredTables = [
        'membership_plans',
    ...
    foreach (
        ...
        if (
            ...
            throw new RuntimeException(
                'Membership campaign database upgrade is incomplete. Missing table: '
                . $table
            );
        }
    }
    ...
    foreach (
        ...
        if (
            ...
            throw new RuntimeException(
                'Membership campaign database upgrade is incomplete. Missing column: '
                . $column
            );
        }
    }
    ...
    foreach (
        ...
        if (
            ...
            throw new RuntimeException(
                'Membership promotion storage is not initialized. Missing column: '
                . $column
            );
        }
    }
...
function membership_admin_checkbox(
    string $key
): int {
    return
        isset($_POST[$key])
...
function membership_admin_month_duration_options(): array
{
    return [
        1,
        2,
...
function membership_admin_month_duration(
    mixed $value
): int {
    $months =
        (int) $value;
    ...
    if (
        !in_array(
            $months,
            membership_admin_month_duration_options(),
            true
        )
    ) {
        throw new InvalidArgumentException(
            'Monthly sale duration must be 1, 2, 3, 6, 9, or 12 months.'
        );
    }
...
function membership_admin_sale_rules_from_post(
    ...
    foreach (
        ...
        if (
            ...
            throw new InvalidArgumentException(
                'Choose a valid discount type for '
                . ucfirst($interval)
                . '.'
            );
        }
        ...
        if (
            ...
            throw new InvalidArgumentException(
                ucfirst($interval)
                . ' dollar discount must be less than the regular price.'
            );
        }


        /*
         * Annual always receives one annual billing period.
         *
         * Monthly receives an explicit number of monthly billing
         * periods: 1, 2, 3, 6, 9, or 12.
         */
        if (
            $interval ===
            LLAMA_MEMBERSHIP_INTERVAL_MONTHLY
            ...
            $durationCount =
                membership_admin_month_duration(
                    $ruleInput[
                        'duration_months'
                    ]
                    ?? 12
            ...
            $duration =
                $durationCount === 1
                    ...
                    : LLAMA_PROMOTION_DURATION_REPEATING;
    ...
    /*
     * Llama Scout promotion policy:
     *
     * Every automatic sale includes Annual.
     * Monthly may be added as a separate offer under the same
     * campaign dates.
     */
    $hasAnnual = false;
...
function membership_admin_create_sale_coupon(
    ...
    /*
     * Monthly:
     * 1 month means the first monthly invoice only.
     * 2, 3, 6, 9, or 12 months repeat for that many invoices.
     *
     * Annual:
     * One annual invoice receives the promotional price.
     */
    if (
        $interval ===
            LLAMA_MEMBERSHIP_INTERVAL_MONTHLY
    ...
    if (
        ...
        throw new RuntimeException(
            'Stripe did not return a Coupon ID for '
            . ucfirst($interval)
            . '.'
        );
    }
...
function membership_admin_discount_label(
    array $rule
): string {
    if (
        (string) (
...
function membership_admin_rule_sale_price(
    array $rule
): int {
    return
...
function membership_admin_rule_duration_label(
    array $rule
): string {
    $interval =
        (string) (
...
function membership_admin_status_label(
    array $promotion
): string {
    if (
        empty(
...
function membership_admin_campaign_month(
    array $promotion
): string {
    try {
        return
...
try {
    membership_admin_require_schema(
        $db
    );
        ...
        try {
            ...
            if (
                ...
                if (
                    ...
                    throw new RuntimeException(
                        'Stripe did not return a new Price ID.'
                    );
                }
            ...
            if (
                ...
                foreach (
                    ...
                    $coupon =
                        membership_admin_create_sale_coupon(
                            $stripe,
                            $promotionId,
                            $name,
                            $rule
                ...
                $notice =
                    'Promotion created and connected to Stripe. Monthly and Annual offers use their own sale rules under the same campaign.';
            }
            ...
            if (
                ...
                foreach (
                    ...
                    /*
                     * Duration is part of the Stripe Coupon.
                     * Changing it requires a fresh Coupon for future
                     * checkouts, just like changing the discount.
                     */
                    $ruleChanged =
                        !$oldRule
                        || (int) (
                ...
                $notice =
                    'Promotion updated. Changed pricing or duration received new Stripe coupons for future checkouts.';
            }
                    ...
                    $db->prepare(
                        'INSERT INTO membership_checkout_settings
                         ...
                         ON DUPLICATE KEY UPDATE
                            manual_promotion_codes_enabled =
                                VALUES(manual_promotion_codes_enabled),
                            updated_by =
                                VALUES(updated_by),
                            updated_at =
                                CURRENT_TIMESTAMP'
...
foreach (
    ...
    $calendarGroups[
        membership_admin_campaign_month(
            $promotion
        )
    ][] =
        $promotion;
...
<form
    class="admin-membership-plan-card"
    method="post"
>
...
<div class="admin-campaign-edit-banner">
    ...
    <span>
        ...
        <strong>
            <?= moderation_e(
                (string) $editingPromotion[
                    'name'
                ]
            ) ?>
        </strong>.

        Monthly and Annual offers can be changed independently.
        Changed pricing or duration creates a new Stripe coupon
...
<form
    class="admin-membership-promotion-form"
    method="post"
>
...
<div class="admin-sale-builder-heading">
    ...
    <div>
        ...
        <p>
            Annual is required for every sale. Monthly is optional
            and can use a different discount and duration under the
            same campaign dates.
        </p>
...
<section
    ...
    data-sale-plan="<?= moderation_e(
        $interval
    ) ?>"
    data-base-cents="<?= (int) $plan[
        'base_price_cents'
    ] ?>"
>
...
<label>
    ...
    <select
        name="sale_rules[<?= $planId ?>][duration_months]"
        data-sale-duration
    >
    ...
    <small class="admin-table-muted">
        The promotional monthly price applies to this many
        monthly billing periods after each customer joins.
    </small>
...
<label>
    ...
    <input
        type="text"
        value="1 annual billing period"
        disabled
    >

    <small class="admin-table-muted">
        The next annual renewal returns to the regular price.
    </small>
...
<div
    class="admin-sale-calculator-row<?= $monthlyPreview[
        'enabled'
    ]
        ? ''
        : ' is-disabled'
...
>
    ...
    <span data-sale-duration-summary>
        <?= $monthlyPreview[
            'enabled'
        ]
            ? 'For '
    ...
    <span data-sale-equivalent>
        ...
        first-year total<?= $monthlyPreview[
            'enabled'
        ]
            ? ''
            : ' at regular price'
...
<div
    class="admin-sale-calculator-row<?= $annualPreview[
        'enabled'
    ]
        ? ''
        : ' is-disabled'
...
>
    ...
    <span data-sale-equivalent>
        ...
        / month equivalent<?= $annualPreview[
            'enabled'
        ]
            ? ''
            : ' at regular price'
...
<label>
    ...
    <textarea
        ...
        rows="3"
        placeholder="Save on Llama Scout when you join during our Halloween promotion."
    ><?= moderation_e(
        (string) (
            $editingPromotion[
                'public_description'
...
<label>
    ...
    <input
        ...
        name="banner_text"
        maxlength="255"
        placeholder="Halloween: Limited-time Llama Scout membership savings"
        value="<?= moderation_e(
            (string) (
                $editingPromotion[
                    'banner_text'
...
<div>
    ...
    <span
        class="admin-status-pill is-<?= moderation_e(
            $status
        ) ?>"
    >
        <?= moderation_e(
...
<a
    class="admin-button"
    href="/memberships.php?edit=<?= (int) $promotion[
        'id'
    ] ?>"
>
    Edit
...
<a
    class="admin-button"
    href="/email-campaigns.php?id=<?= (int) $promotion[
        'id'
    ] ?>"
>
    Campaign emails
...
$salePrice =
    membership_admin_rule_sale_price(
        $rule
    );

$durationLabel =
    membership_admin_rule_duration_label(
        $rule
    );
...
<span>
    ...
    <?= moderation_e(
        membership_admin_discount_label(
            $rule
        )
    ) ?>
...
<form
    class="admin-membership-code-setting"
    method="post"
>
...
<label>
    ...
    Allow Stripe promotion codes at membership checkout when no
    automatic promotion is active.
