...
function llama_public_profile_by_username(PDO $db, string $username): ?array
{
    ...
    $stmt = $db->prepare(
        'SELECT
         ...
         FROM users u
         INNER JOIN community_profiles cp ON cp.user_id = u.id
    ...
    $profile['images'] = llama_community_profile_images($db, (int) $profile['id']);
    $profile['badges'] = llama_user_badges($db, (int) $profile['id']);
    $profile['primary_image'] = llama_primary_profile_image($db, (int) $profile['id']);

    return $profile;
}

function llama_public_profiles(PDO $db, int $limit = 100): array
{
    ...
    $stmt = $db->query(
        'SELECT
            u.id, u.username, u.display_name,
            cp.bio, cp.location, cp.primary_image_id,
            COALESCE(i.image_src, \'' . LLAMA_DEFAULT_PROFILE_IMAGE . '\') AS primary_image
         FROM users u
         INNER JOIN community_profiles cp ON cp.user_id = u.id
         LEFT JOIN community_profile_images i
           ON i.id = cp.primary_image_id
          AND i.user_id = u.id
         WHERE u.status = \'active\'
           AND cp.is_public = 1
         ORDER BY cp.updated_at DESC, u.display_name ASC
         LIMIT ' . $limit
    );
...
function llama_profile_save(PDO $db, int $userId, array $input): void
{
    ...
    $fields = [
        ...
        'bluesky_url' => trim((string) ($input['bluesky_url'] ?? '')),
    ...
    foreach ($urlFields as $field) {
        if ($fields[$field] !== '' && !filter_var($fields[$field], FILTER_VALIDATE_URL)) {
            throw new InvalidArgumentException('Social and website links must be complete URLs, including https://.');
        }
    }
