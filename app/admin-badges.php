...
function admin_badges_user_badges(PDO $db, int $userId): array {
    $stmt = $db->prepare(
        'SELECT
            ub.*,
            bd.slug,
...
function admin_badges_user_badge(PDO $db, int $userId, int $userBadgeId): ?array {
    if ($userId < 1 || $userBadgeId < 1) {
        return null;
    }
    ...
    $stmt->execute([$userBadgeId, $userId]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    return $row ?: null;
}

function admin_badges_stats(PDO $db): array {
    $row = $db->query(
        'SELECT
            (
                SELECT COUNT(*)
...
function admin_badges_validate_url(string $value): ?string {
    $value = trim($value);
    if ($value === '') {
        return null;
    }
    ...
    if (!filter_var($value, FILTER_VALIDATE_URL)) {
        throw new RuntimeException('Enter a valid evidence URL.');
    }
    ...
    if (!in_array(
        ...
        throw new RuntimeException('Evidence links must use HTTP or HTTPS.');
    }
...
function admin_badges_save_definition(
    ...
    $existing = $badgeId > 0
        ? admin_badges_definition($db, $badgeId)
        : null;

    if ($badgeId > 0 && !$existing) {
        throw new RuntimeException('Badge definition not found.');
    }
    ...
    if (mb_strlen($name) > 150) {
        throw new RuntimeException('Badge name must be 150 characters or fewer.');
    }

    $incomingSlug = trim((string) ($data['slug'] ?? ''));
    $slug = admin_badges_slugify($incomingSlug !== '' ? $incomingSlug : $name);
    if ($slug === '') {
        throw new RuntimeException('Badge slug is required.');
    }
    if (strlen($slug) > 100) {
        throw new RuntimeException('Badge slug must be 100 characters or fewer.');
    }
    ...
    $duplicate->execute([$slug, $badgeId]);
    if ($duplicate->fetchColumn()) {
        throw new RuntimeException('That badge slug is already in use.');
    }

    $description = trim((string) ($data['description'] ?? ''));
    if (mb_strlen($description) > 500) {
        throw new RuntimeException('Badge description must be 500 characters or fewer.');
    }

    $category = strtolower(trim((string) ($data['category'] ?? 'community')));
    if (!in_array(
        $category,
        ['community', 'scouting', 'stewardship', 'training', 'special'],
        true
    )) {
        throw new RuntimeException('Choose a valid badge category.');
    }

    $awardType = strtolower(trim((string) ($data['award_type'] ?? 'manual')));
    if (!in_array($awardType, ['automatic', 'manual', 'credential'], true)) {
    ...
    if ($badgeId > 0) {
        ...
        admin_users_audit(
            ...
            'Updated badge definition "' . $name . '".',
            [
                'badge_id' => $badgeId,
                'before_slug' => $existing['slug'] ?? null,
                'after_slug' => $slug,
                'before_active' => isset($existing['is_active'])
...
function admin_badges_award(
    ...
    if ($userId < 1 || $badgeId < 1) {
        throw new RuntimeException('Choose a member and badge.');
    }

    $badge = admin_badges_definition($db, $badgeId);
    if (!$badge) {
        throw new RuntimeException('Badge definition not found.');
    }
...
function admin_badges_replace_image_from_stage(
    ...
    $slug = admin_badges_slugify((string) $badge['slug']);
    if ($slug === '') {
        throw new RuntimeException(
    ...
    $sourcePath = (string) ($committed[0]['path'] ?? '');
    if ($sourcePath === '') {
